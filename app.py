...
# # Loading the data
# CC_df = pd.read_csv(r"C:\Users\chamo\Desktop\credit\Credit-card-fraud-detection\CC_fraud_detection\creditcard.csv")
...
# Dataset Selection
st.markdown("<h3 style='color:blue;'>Upload or Use Default Dataset:</h3>", unsafe_allow_html=True)
uploaded_file = st.file_uploader("Upload your CSV file here", type=["csv"])

# Default dataset path
default_path = r"C:\Users\chamo\Desktop\credit\Credit-card-fraud-detection\CC_fraud_detection\creditcard.csv"

if uploaded_file:
    try:
        # Load the uploaded dataset
        CC_df = pd.read_csv(uploaded_file)
        st.success("Dataset successfully loaded from the uploaded file.")
    except Exception as e:
        st.error(f"Error loading the uploaded dataset: {e}")
elif default_path:
    try:
        # Load the default dataset
        CC_df = pd.read_csv(default_path)
        st.success("Default dataset loaded successfully.")
    except Exception as e:
        st.error(f"Error loading the default dataset: {e}")
else:
    st.error("No dataset available! Please upload a file or check the default dataset path.")

# Proceed if dataset is loaded
if 'CC_df' in locals():
    # Display the first few rows of the dataset
    st.markdown("<h4>Preview of the Dataset:</h4>", unsafe_allow_html=True)
    st.write(CC_df.head())
    ...
    # Balancing legit and fraud data
    legit_sample = legit.sample(n=len(fraud), random_state=2)  # Balancing the dataset
    cc_df2 = pd.concat([legit_sample, fraud], axis=0)
    ...
    st.markdown(f"**Training Accuracy:** {train_acc:.2f}")
    st.markdown(f"**Testing Accuracy:** {test_acc:.2f}")

    # User Interaction Section
    st.markdown("<h3 style='color:blue;'>Select a Random Sample from the Dataset:</h3>", unsafe_allow_html=True)
    
    # Initialize session state if not done already
    if 'random_sample' not in st.session_state:
        st.session_state.random_sample = None

    # Button to fetch a random sample
    if st.button("Fetch Random Sample"):
        # Fetch a random sample from X_test (ensure it's in DataFrame format)
        st.session_state.random_sample = pd.DataFrame(X_test).sample(n=1, random_state=None)
        st.write("Random Sample Features:")
        st.write(st.session_state.random_sample)

    # Check if a random sample exists to predict
    if st.session_state.random_sample is not None:
        # Convert the random sample to a format suitable for prediction
        sample_array = st.session_state.random_sample.iloc[0].values  # Use .iloc[0] to get the first row as a NumPy array
        sample_input = ', '.join(map(str, sample_array))

        # Display prefilled input box with sample data
        input_data = st.text_area("Modify or Confirm Features Below:", sample_input)
        
        # Submit for prediction
        if st.button("Predict Transaction"):
            try:
                # Convert input string back to numpy array
                ...
                st.error("Error: Please ensure all inputs are valid numbers.")
            except Exception as e:
